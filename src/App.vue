<template>
  <div>
    <strong v-if="careerfirstChanges !== 'careerfirst'" style="color: #010127; font-size: medium">
      <button text fab small size="3vw" style="margin-right: 7px" @click="showQrCodeScanner = true" :disabled="sppuInstituteIdFlag">
    ...
    <v-dialog v-model="showQrCodeScanner" v-if="showQrCodeScanner === true" width="auto">
      <v-card>
        <v-card-title>
          Scanner--1
          <v-spacer></v-spacer>
          <button text fab small size="3vw" style="margin-right: 7px" @click="toggleCamera">
        ...
        <v-card-text>
          <div id="app">
            ...
            <div>
              ...
              <input id="zoom" type="range" min="1" max="5" step="0.1" v-model="zoomLevel" @input="adjustZoom" />
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <button color="#FFFFFF" text @click="showQrCodeScanner = false" style="background-color: #ff4f1f; margin: 0 auto; display: block">
            Ok
          </button>
        </v-card-actions>
      </v-card>
